...
{
	struct regulator *panel_reset = NULL;
	...
	panel_reset = regulator_get(NULL, ts->pdata->panel_reset);
...
{
	struct regulator *panel_buck_en = NULL;
	struct regulator *panel_buck_en2 = NULL;
	struct regulator *panel_ldo_en = NULL;
	...
	panel_buck_en = regulator_get(NULL, ts->pdata->panel_buck_en);
	...
	panel_buck_en2 = regulator_get(NULL, ts->pdata->panel_buck_en2);
	if (IS_ERR_OR_NULL(panel_buck_en)) {
	...
	panel_ldo_en = regulator_get(NULL, ts->pdata->panel_ldo_en);
...
error:
	regulator_put(panel_buck_en);
	regulator_put(panel_buck_en2);
	regulator_put(panel_ldo_en);
